...
class PrerenderHTMLWebpackPlugin {
  ...
  async process(compiler, compilation, htmlPluginData) {
    ...
    let renderer = isCustomRenderMethodProvided
      ? () => Document(htmlPluginData)
      : () => renderReactPage(Document, htmlPluginData);

    htmlPluginData.html = htmlPluginData.html.trim() + (await renderer());
